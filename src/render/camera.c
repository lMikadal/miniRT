...
t_ray	get_ray(double u, double v, t_ca camera)
{
	...
	group_d_2 = v3d_opr_minus(\
					v3d_mult_double(camera.vertical, v), camera.origin);
	direct = v3d_opr_plus(group_d_1, group_d_2);
	return (ray_create(camera.origin, direct));
}

t_ca	create_camera(t_v3d lookfrom, t_v3d lookat, t_v3d vup, t_ca_unit unit)
{
	t_ca	camera;
	double	view[2];
	t_v3d	w;
	...
	view[0] = 2.0 * tan((unit.fov * PI / 180.0) / 2);
	view[1] = view[0] / unit.ratio;
	...
	camera.l_l_c = v3d_opr_minus(v3d_opr_minus(\
			v3d_opr_minus(camera.origin, v3d_div_double(camera.horizontal, 2)), \
